...
Database.prototype.version = function(valueOrCallback) {
  if (typeof valueOrCallback === 'function') {
    ...
    return this.get('PRAGMA user_version', undefined, undefined, Database.RESULTSASOBJECT);
  }
};
    ...
    for (var i = 0; i < count; ++i) {
      if (params[i] == null) { // jshint ignore:line
        ...
        if (typeof params[i] == 'number') {
          ...
          res = sqlite3_bind_text(statement, i + 1, params[i], -1, null);
        }
      }
      if (res) {
        var errMsg = sqlite3_errmsg(self._db);
